...
export class CommentsService {
  ...
  async findOne(param: IdParam) {
    let commentFound = await this.commentModel
      .findById(param.id)
      .populate('author')
      .populate('post')
      .populate('likes')
  ...
  async update(
    ...
    return this.commentModel.updateOne(
      ...
      { $set: { ...updateCommentDTO, lastUpdate: new Date() } },
    )
  }
  ...
  async getNbCommentsOfAPost(param: IdParam) {
    return this.commentModel.countDocuments({ post: param.id })
  }
}
