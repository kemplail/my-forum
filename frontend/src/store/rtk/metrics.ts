...
export const metricsApi = emptySplitApi.injectEndpoints({
  endpoints: (builder) => ({
    getEvolutionNbPosts: builder.query<
      { data: [string, string | number][]; countMax: number },
      string
    >({
      query: (date) => ({
      ...
      transformResponse: (defaultResponse: NbPostsEvolution[]) => {
        let xAxis: string[] = [];
        let yAxis: number[] = [];
        let response: { data: [string, string | number][]; countMax: number } =
          { data: [["Date", "Nombre de posts"]], countMax: 0 };

        if (defaultResponse) {
          xAxis = defaultResponse.map(
            (element) =>
    ...
    getEvolutionNbLikes: builder.query<
      { data: [string, string | number][]; countMax: number },
      string
    >({
      query: (date) => ({
      ...
      transformResponse: (defaultResponse: NbLikesEvolution[]) => {
        ...
        let yAxis: number[] = [];
        let response: { data: [string, string | number][]; countMax: number } =
          { data: [["Date", "Nombre de likes"]], countMax: 0 };
    ...
    //getNbPostsPerUser
    getNbPostsPerUser: builder.query<
    ...
    >({
      ...
      transformResponse: (defaultResponse: NbPostsPerUser[]) => {
        let xAxis: string[] = [];
        let yAxis: number[] = [];
        let response: { data: [string, string | number][] } = {
          data: [["Username", "Nombre de posts"]],
        };
    ...
    getWordCloud: builder.query<Word[], void>({
      ...
      transformResponse: (defaultResponse: Record<string, number>) => {
        let newResponse = Object.keys(defaultResponse).map((key) => ({
          text: key,
          value: defaultResponse[key] * 100,
        }));
        return newResponse;
      },
    }),
